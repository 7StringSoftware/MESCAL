...
void EffectGraphComponent::setOutputEffect(mescal::Effect::Ptr outputEffect)
{
    viewportContent.buildEffectGraphComponents(outputEffect);

    setCallbackRecursive(outputEffect, [this]()
        {
            if (onEffectPropertyChange)
            {
...
void EffectGraphComponent::ViewportContent::buildEffectGraphComponents(mescal::Effect::Ptr newOutputEffect)
{
...
//     {
//         effectComponent->thumbnail = juce::Image{ juce::Image::ARGB, imageWidth, imageHeight, true, juce::NativeImageType{} };
//     }
...
{
    auto propertyPanel = std::make_unique<EffectPropertyPanel>(effect);
    juce::Component::SafePointer<juce::Component> that{ this };
    propertyPanel->onPropertyChange = [=]()
        {
            if (auto parent = getParentComponent())
                if (!parent->getBounds().isEmpty())
                    parent->repaint();
        };
    juce::CallOutBox::launchAsynchronously(std::move(propertyPanel),
        getScreenBounds(),
        nullptr);
}
...
void EffectGraphComponent::EffectComponent::paint(juce::Graphics& g)
{
    ...
    if (thumbnail.isNull())
    {
        thumbnail = juce::Image{ juce::Image::ARGB, 300, 300, true };
    }

    auto dest = getLocalBounds().withTrimmedBottom(textHeight).toFloat();
    effect->applyEffect(thumbnail, {}, true);
    g.drawImageTransformed(thumbnail, juce::RectanglePlacement{}.getTransformToFit(thumbnail.getBounds().toFloat(), dest));
