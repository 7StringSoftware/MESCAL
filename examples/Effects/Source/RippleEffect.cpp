...
#define CAT(X, Y) X##Y
#define XML(X) TEXT(CAT(L, # X))

RippleEffect::RippleEffect() :
    m_refCount(1)
{
    m_constants.amplitude = 10.0;
    m_constants.center = D2D1::Point2F(100.0f, 100.0f);
    m_constants.frequency = 10.0f;
    m_constants.phase = 0;
    m_constants.spread = 100.0f;
}
...
void RippleEffect::registerSelf()
{
    juce::SharedResourcePointer<juce::DirectX> dx;
    Register(dx->getD2DFactory());
}
    ...
    )
{
...
#if 0
    std::unique_ptr<BasicReaderWriter> reader = std::make_unique<BasicReaderWriter>();
    ...
    juce::File csoFile{ "C:/proj/MESCAL/examples/Effects/Source/RippleEffect.cso" };
    juce::MemoryBlock block;
    csoFile.loadFileAsData(block);

    HRESULT hr = pEffectContext->LoadPixelShader(GUID_RipplePixelShader, (BYTE*)block.getData(), (uint32_t)block.getSize());
